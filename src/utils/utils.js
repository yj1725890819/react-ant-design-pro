...
export const importCDN = (url, name) =>
  ...
  {
    if(!pageRange){return '分页错误';}
    const totalPage=Math.ceil(pageNum/pageSize);
    const currenPage=Math.ceil(pageRange[0]/pageSize);
    return `总共${pageNum}条,第${currenPage}/${totalPage}页`;
  }
export function getDicOptions(dicCategoryNos)
{
  if(!dicCategoryNos.length) return;
  const params={cachebatchOne: dicCategoryNos};
  return request('wholeflow/com/cachebatchOne',{
    method:'POST',
    body:params,
  }).then(response=>{
    ...
    const options=response.RsponseBody.retcacheList;
    const obj={};
    for(let i=0;i<dicCategoryNos.length;i++){
      for(let j=0;j<options.length;j++)
      {
        if(options[j].dictionaryCategoryNo===dicCategoryNos[i].dictionaryCategoryNo)
        {
          ...
          {
            obj[options[j].dictionaryCategoryNo]=options[j].dictionaries.filter(itemTmp=>{
              return dicCategoryNos[i].NotInArr.every(
                item=>item.dictionaryNo!==itemTemp.dictionaryNo
              );
            });
          }else {
...
export function showErrorMsg(response)
{
  ...
  Modal.err({
   ...
   content:(
     ...
     错误码：
     <span style={{color:'#f5222d'}}>{response.fault.faultCode}</span>
     <br/>
     错误信息：
     <span style={{color:'#f5222d'}}>{response.fault.faultMessage}</span>
     </div>
   ),
  });
}
